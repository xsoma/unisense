...
void DoClantag()
{
	...
	if (vars.visuals.clantagspammer)
	{
		...
		if (int(interfaces.global_vars->curtime * 2 + latency) != iLastTime)
		{
			...
			{
				switch (int(interfaces.global_vars->curtime * 2.4) % 11) {
				case 0: SetClanTag(hs::clantag1::s().c_str(), hs::clantag1::s().c_str()); break;
				case 1: SetClanTag(hs::clantag2::s().c_str(), hs::clantag2::s().c_str()); break;
				case 2: SetClanTag(hs::clantag3::s().c_str(), hs::clantag3::s().c_str()); break;
				case 3: SetClanTag(hs::clantag4::s().c_str(), hs::clantag4::s().c_str()); break;
				case 4: SetClanTag(hs::clantag5::s().c_str(), hs::clantag5::s().c_str()); break;
				case 5: SetClanTag(hs::clantag6::s().c_str(), hs::clantag6::s().c_str()); break;
				case 6: SetClanTag(hs::clantag7::s().c_str(), hs::clantag7::s().c_str()); break;
				case 7: SetClanTag(hs::clantag8::s().c_str(), hs::clantag8::s().c_str()); break;
				case 8: SetClanTag(hs::clantag9::s().c_str(), hs::clantag9::s().c_str()); break;
				case 9: SetClanTag(hs::clantag10::s().c_str(), hs::clantag10::s().c_str()); break;
				case 10: SetClanTag(hs::clantag11::s().c_str(), hs::clantag11::s().c_str()); break;
				}
			}
...
void ProcessMissedShots()
{
	...
	if (fabs((csgo->local->GetTickBase() * interfaces.global_vars->interval_per_tick) - snapshot.time) > 1.f)
	{
		if (snapshot.weapon_fire) {
			if (snapshot.bullet_impact)
				Msg("Can't proceed shot", color_t(vars.visuals.eventlog_color));
			else
				Msg("Can't proceed impact", color_t(vars.visuals.eventlog_color));
		}
	...
	if (snapshot.first_processed_time != -1.f) {
		if (snapshot.damage == -1 && snapshot.weapon_fire && snapshot.bullet_impact && snapshot.record->player) {
			string msg;
			msg += "Missed shot due to " + snapshot.entity->GetName();
			...
			Msg(msg, color_t(vars.visuals.eventlog_color));
			shot_snapshots.erase(shot_snapshots.begin());
		}
	}
}
...
void InitDoubleTap()
{
	...
	if (CanDT() && !csgo->game_rules->IsFreezeTime())
	{
		if (vars.ragebot.double_tap->active)
		{
			...
			auto can_shift_shot = CMAntiAim::Get().can_exploit(16 + vars.ragebot.dt_speed_boost / 10);
			auto can_shot = CMAntiAim::Get().can_exploit(abs(-1 - prev_shift_ticks));

			if (can_shift_shot || !can_shot && !did_shift_before)
			{
				prev_shift_ticks = 16 + vars.ragebot.dt_speed_boost / 10;
				double_tapped = 0;
			}
			else {
				double_tapped++;
			...
			csgo->TickShifted = false;
			csgo->nSinceUse = 0;
			if (vars.ragebot.dt_delay / 10 - correcttickbase() <= 1 * interfaces.global_vars->interval_per_tick)
			{
...
bool __stdcall Hooked_CreateMove(float a, CUserCmd* cmd) {
	...
	if (csgo->local->isAlive()) {
		...
		if (vars.misc.airstuckkey->active)
		{
