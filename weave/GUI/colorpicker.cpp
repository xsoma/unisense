...
void c_colorpicker::render() {
	...
	g_Render->filled_rect_gradient(pos.x + 1, pos.y + 1, size.x - 2, size.y - 2,
		clr, clr,
	...
	if (dialogue.open) {
		...
		for (size_t i = 0; i < dialogue.elements.size(); i++) {
			...
			g_Render->FilledRect(pos.x, pos.y, size.x, size.y,
				color_t(25.f + 15.f * dialogue.animations[i], 25.f + 15.f * dialogue.animations[i], 25.f + 15.f * dialogue.animations[i], alpha));
		...
		g_Render->FilledRect(pos.x + 1, pos.y + 1, size.x - 2, 163, color_t(25, 25, 25, alpha));
		...
		g_Render->filled_rect_gradient(pos.x + 15, pos.y + 145, size.x - 30.f, 10.f,
			color_t(25, 25, 25, 0), clr, clr, color_t(25, 25, 25, 0));
