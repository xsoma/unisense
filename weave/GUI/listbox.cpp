...
void c_listbox::render() {
	...
	g_Render->FilledRect(pos.x, pos.y, size.x, size.y,
		color_t(25, 25, 25, alpha));
	if (should_draw_scroll)
		pos.y -= scroll;
	for (int i = 0; i < elements.size(); i++) {
		g_Render->FilledRect(pos.x, pos.y, size.x, 20,
			color_t(35 + 10.f * animations[i], 35 + 10.f * animations[i], 35 + 10.f * animations[i], alpha));

		/*g_Render->DrawString(pos.x + 10, pos.y + 10,
			*(unsigned int*)value == i ? color_t(200 + 55.f * animations[i], 100 + 50.f * animations[i], 25.f - 25.f * animations[i], alpha) :
		...
		g_Render->DrawString(pos.x + 10, pos.y + 10,
			*(unsigned int*)value == i ? color_t(84 + 55.f * animations[i], 25.f - 25.f * animations[i], 200 + 50.f * animations[i], alpha) :
			color_t(200 + 55 * animations[i], 200 + 55 * animations[i], 200 + 55 * animations[i], alpha), render::centered_y,
			fonts::menu_desc, elements[i].c_str());
