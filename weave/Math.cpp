#include "Math.h"
Vector Math::normalize(Vector angs)
{
	while (angs.y < -180.0f)
		angs.y += 360.0f;
	while (angs.y > 180.0f)
...
void Math::AngleMatrix(const Vector &angles, matrix &matrix)
{
...
void Math::MatrixSetColumn(const Vector &in, int column, matrix &out)
{
	out[0][column] = in.x;
	out[1][column] = in.y;
	out[2][column] = in.z;
...
void Math::AngleMatrix(const Vector & angles, const Vector & position, matrix & matrix)
{
...
void Math::RotateMatrix(matrix in[128], matrix out[128], float delta, Vector renderOrigin) {
	auto vDelta = Vector(0, delta, 0);
	Vector vOutPos;
	for (int i = 0; i < 128; i++)
	{
		Math::AngleMatrix(vDelta, out[i]);
		Math::MatrixMultiply(out[i], in[i]);
		auto vBonePos = Vector(in[i][0][3],
			in[i][1][3],
			in[i][2][3]) - renderOrigin;
		vOutPos = Math::vector_rotate(vBonePos, vDelta);
...
void Math::MatrixMultiply(matrix& in1, const matrix& in2)
{
	matrix out;
	if (&in1 == &out)
	{
...
//	matrix[2][3] = 0.0f;
//}
const matrix& Math::world_to_screen_matrix()
{
...
bool Math::WorldToScreen2(Vector &in, Vector &out) // (Vector &in, Vector &out)
{
...
void Math::VectorTransform(const Vector& in1, const matrix& in2, Vector& out) {
...
bool Math::IsNearEqual(float v1, float v2, float Tolerance)
{
...
void Math::VectorTransform(const float *in1, const matrix& in2, float *out)
{
...
void Math::VectorTransform_Wrapper(const Vector& in1, const matrix &in2, Vector &out)
{
...
Vector Math::vector_rotate(Vector& in1, matrix& in2)
{
...
void  Math::VectorRotate(const float* in1, const matrix& in2, float* out)
{
...
void  Math::VectorRotate(const Vector& in1, const matrix& in2, Vector& out)
{
...
Vector Math::vector_rotate(Vector& in1, Vector& in2)
{
	matrix m;
	Math::AngleMatrix(in2, m);
	return vector_rotate(in1, m);
}

void Math::vector_i_rotate(Vector in1, matrix in2, Vector& out)
{
...
void Math::vector_i_transform(const Vector& in1, const matrix& in2, Vector& out)
{
...
void Math::AngleVectorsDefault(Vector q, Vector& vec)
{
	float sp, sy, cp, cy;
...
void Math::AngleVector4(const Vector &angles, Vector& forward, Vector& right, Vector& up)
{
	float sr, sp, sy, cr, cp, cy;
...
void Math::VectorAngles(const Vector &forward, const Vector &pseudoup, Vector &angles)
{
...
float Math::DistanceScreen(Vector2D vDstPos, Vector2D vSrcPos)
{
...
void Math::SmoothAngles(Vector MyViewAngles, Vector AimAngles, Vector &OutAngles, float Smoothing)
{
...
Vector Math::CalculateAngle(Vector src, Vector dst)
{
	Vector angles;
...
void Math::SetMatrixOrigin(Vector pos, matrix& pMat) {
	for (size_t i{ }; i < 3; ++i) {
		pMat[i][3] = pos[i];
	}
}

Vector Math::GetMatrixOrigin(const matrix& src) {
	return Vector(src[0][3], src[1][3], src[2][3]);
}
void Math::VectorTransform2(const Vector in1, matrix in2, Vector& out)
{
