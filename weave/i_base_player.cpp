...
{
	Vector eyePos(0.f, 0.f, 0.f);
	ShootPos(&eyePos);
	return eyePos;
}

Vector IBasePlayer::GetHitboxPos(int hitbox)
{
	matrix boneMatrix[256];

	if (this->SetupBones(boneMatrix, 128, 0x00000100, this->GetSimulationTime()))
	{
		studiohdr_t* hdr = interfaces.models.model_info->GetStudioModel(this->GetModel());
		mstudiohitboxset_t* set = hdr->pHitboxSet(0);
		mstudiobbox_t* hit_box = set->pHitbox(hitbox);
		Vector
			bbmin = Vector{},
			bbmax = Vector{};

		Math::VectorTransform(hit_box->bbmin, boneMatrix[hit_box->bone], bbmin);
		Math::VectorTransform(hit_box->bbmax, boneMatrix[hit_box->bone], bbmax);
...
{
	return *(float*)((DWORD)this + 0xA410);
}
